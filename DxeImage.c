...
#define CoreLocateHandle gBS->LocateHandle
#define CoreHandleProtocol gBS->HandleProtocol
#define CoreAllocatePages AllocatePages
#define CoreFreePages FreePages
#define CoreFreePool FreePool
#define CoreInstallProtocolInterface gBS->InstallProtocolInterface
#define CoreCreateEvent gBS->CreateEvent
#define CoreRegisterProtocolNotify gBS->RegisterProtocolNotify
#define CoreLocateDevicePath gBS->LocateDevicePath
#define CoreLocateHandleBuffer gBS->LocateHandleBuffer
#define CoreProtocolsPerHandle gBS->ProtocolsPerHandle
#define CoreOpenProtocolInformation gBS->OpenProtocolInformation
#define CoreCloseProtocol gBS->CloseProtocol
#define CoreRemoveDebugImageInfoEntry gBS->RemoveDebugImageInfoEntry
#define CoreUninstallProtocolInterface gBS->UninstallProtocolInterface
#define CoreRestoreTpl gBS->RestoreTPL
#define CoreRaiseTpl gBS->RaiseTPL
#define CoreReinstallProtocolInterface gBS->ReinstallProtocolInterface
    ...
    for (Index = 0; Index < NumberOfSections; Index++) {
        ...
        if ((SectionHeader.Characteristics & EFI_IMAGE_SCN_CNT_CODE) == 0) {
            ...
            if (ValueInSectionHeader != 0) {
                ...
                //
                Status = CheckAndMarkFixLoadingMemoryUsageBitMap(ImageContext->ImageAddress,
...
//
    InvalidateInstructionCacheRange((VOID *) (UINTN) Image->ImageContext.ImageAddress,
...
//
    if (SetJumpFlag == 0) {
        RegisterMemoryProfileImage(Image, (Image->ImageContext.ImageType == EFI_IMAGE_SUBSYSTEM_EFI_APPLICATION
                                           ? EFI_FV_FILETYPE_APPLICATION : EFI_FV_FILETYPE_DRIVER));
//
